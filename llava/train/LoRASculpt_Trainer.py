...
RISK_TOPK_RATIO = float(os.environ.get("RISK_TOPK_RATIO", 0.2))
RISK_WARMUP_STEPS = int(os.environ.get("RISK_WARMUP_STEPS", 100))
RISK_MIN_KEEP_RANK = int(os.environ.get("RISK_MIN_KEEP_RANK", 1))
RISK_LAYER_RELAX_MM = float(os.environ.get("RISK_LAYER_RELAX_MM", 0.5))
RISK_PATCH_TRIGGER = float(os.environ.get("RISK_PATCH_TRIGGER", 0.25))
RISK_PATCH_RANK_MAX = int(os.environ.get("RISK_PATCH_RANK_MAX", 16))
...
class LoRASculpt(LLaVATrainer):
    ...
    def _ensure_risk_state(self):
        ...
        self.risk_masks = {}
        self.risk_ema = {}
        self.layer_block_ratio = {}
        self.risk_log_history = []

    @staticmethod
    def _is_target_lora_layer(name: str) -> bool:
        return any(k in name for k in ("q_proj", "k_proj", "v_proj", "mm_projector"))

    @staticmethod
    def _base_layer_key_from_lora_name(name: str) -> str:
        if ".lora_A.default.weight" in name:
            return name.replace(".lora_A.default.weight", "")
        if ".lora_B.default.weight" in name:
            return name.replace(".lora_B.default.weight", "")
        if ".lora_A.weight" in name:
            return name.replace(".lora_A.weight", "")
        if ".lora_B.weight" in name:
            return name.replace(".lora_B.weight", "")
        return name

    def _collect_rank_grad_conflict_scores(self, model: nn.Module):
        scores = {}
        params = dict(model.named_parameters())
        eps = 1e-8
        for name, A in params.items():
            if "lora_A" not in name or not self._is_target_lora_layer(name):
                continue
            base_key = self._base_layer_key_from_lora_name(name)
            if ".lora_A.default.weight" in name:
                b_name = f"{base_key}.lora_B.default.weight"
            else:
                b_name = f"{base_key}.lora_B.weight"
            ...
            a_signed = torch.mean(gA, dim=1)
            b_signed = torch.mean(gB, dim=0)
            cos_proxy = (a_signed * b_signed) / ((torch.abs(a_signed) + eps) * (torch.abs(b_signed) + eps))
            conflict = (1.0 - cos_proxy) * 0.5
            strength = torch.sqrt(a_norm * b_norm)
            score = conflict * strength
            scores[base_key] = score.float()
        return scores

    def _update_risk_masks(self, model: nn.Module, global_step: int):
        self._ensure_risk_state()
        new_scores = self._collect_rank_grad_conflict_scores(model)
        if not new_scores:
            return
        logs = {"step": int(global_step), "layers": {}}
        for layer, score in new_scores.items():
            prev = self.risk_ema.get(layer)
            if prev is None or prev.shape != score.shape:
                ema = score
            else:
                ema = RISK_EMA_BETA * prev + (1.0 - RISK_EMA_BETA) * score
            self.risk_ema[layer] = ema

            rank = ema.numel()
            keep_min = max(1, min(rank, RISK_MIN_KEEP_RANK))
            base_ratio = RISK_TOPK_RATIO
            if any(k in layer for k in ("mm_projector", "q_proj", "v_proj")):
                base_ratio = base_ratio * RISK_LAYER_RELAX_MM
            prune_k = int(rank * max(0.0, min(0.95, base_ratio)))
            prune_k = min(prune_k, rank - keep_min)

            mask = torch.ones(rank, device=ema.device, dtype=ema.dtype)
            if global_step >= RISK_WARMUP_STEPS and prune_k > 0:
                top_idx = torch.topk(ema, k=prune_k, largest=True).indices
                mask[top_idx] = 0.0
            self.risk_masks[layer] = mask
            block_ratio = float((mask == 0).float().mean().item())
            self.layer_block_ratio[layer] = block_ratio
            logs["layers"][layer] = {
                "blocked": int((mask == 0).sum().item()),
                "rank": int(rank),
                "block_ratio": block_ratio,
                "risk_mean": float(ema.mean().item()),
            }
        self.risk_log_history.append(logs)
        if logs["layers"]:
            preview = ", ".join(
                [f"{k}: {v['blocked']}/{v['rank']}" for k, v in list(logs["layers"].items())[:4]]
            )
            logger.info(f"[RiskMask] step={global_step} {preview}")

    def _apply_risk_masks_to_grads(self, model: nn.Module):
        if not getattr(self, "risk_masks", None):
            return
        params = dict(model.named_parameters())
        for name, A in params.items():
            if "lora_A" not in name:
                continue
            layer = self._base_layer_key_from_lora_name(name)
            mask = self.risk_masks.get(layer)
            if mask is None:
                continue
            if ".lora_A.default.weight" in name:
                b_name = f"{layer}.lora_B.default.weight"
            else:
                b_name = f"{layer}.lora_B.weight"
            if b_name not in params:
                continue
            B = params[b_name]
            if A.grad is not None:
                A.grad.mul_(mask.to(A.grad.device, dtype=A.grad.dtype).view(-1, 1))
            if B.grad is not None:
                B.grad.mul_(mask.to(B.grad.device, dtype=B.grad.dtype).view(1, -1))

    def _apply_risk_masks_to_params(self, model: nn.Module):
        if not getattr(self, "risk_masks", None):
            return
        params = dict(model.named_parameters())
        for name, A in params.items():
            ...
            B = params[b_name]
            with torch.no_grad():
                A.mul_(mask.to(A.device, dtype=A.dtype).view(-1, 1))
                B.mul_(mask.to(B.device, dtype=B.dtype).view(1, -1))

    def _clear_masked_optimizer_state(self, model: nn.Module):
        if not getattr(self, "risk_masks", None) or self.optimizer is None:
            return
        params = dict(model.named_parameters())
        for name, A in params.items():
            if "lora_A" not in name:
            ...
            if b_name not in params:
                continue
            B = params[b_name]
            for p, is_a in ((A, True), (B, False)):
                st = self.optimizer.state.get(p, None)
                if not st:
                    continue
                for k in ("exp_avg", "exp_avg_sq"):
                    if k not in st or st[k] is None:
                        continue
                    m = mask.to(st[k].device, dtype=st[k].dtype)
                    if is_a:
                        st[k].mul_(m.view(-1, 1))
                    else:
                        st[k].mul_(m.view(1, -1))
    ...
    def mount_residual_patches(self, model, patch_rank=4):
        ...
        for name, module in model.named_modules():
            if any(k in name for k in target_layer_keywords) and isinstance(module, nn.Linear):
                cur_patch_rank = patch_rank
                if RISK_MASK_ENABLE and getattr(self, "layer_block_ratio", None):
                    layer_ratio = self.layer_block_ratio.get(name, 0.0)
                    if layer_ratio < RISK_PATCH_TRIGGER:
                        continue
                    scaled_rank = int(max(1, round(patch_rank * (1.0 + layer_ratio))))
                    cur_patch_rank = min(scaled_rank, RISK_PATCH_RANK_MAX)
                patch_alpha = float(os.environ.get('PATCH_ALPHA', 1.0))
                patch = ResidualPatch(
                    module.in_features, module.out_features,
                    rank=cur_patch_rank, alpha=patch_alpha,
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model): # automatically perform the gradient accumulation
                    ...
                    if RISK_MASK_ENABLE:
                        if (self.state.global_step + 1) % max(1, RISK_UPDATE_INTERVAL) == 0:
                            self._update_risk_masks(model, self.state.global_step)
                        self._apply_risk_masks_to_grads(model)
                ...
                if (
                    ...
                    self.optimizer.step()
                    if RISK_MASK_ENABLE:
                        self._apply_risk_masks_to_params(model)
                        self._clear_masked_optimizer_state(model)
        ...
        if RISK_MASK_ENABLE and getattr(self, "risk_log_history", None):
            try:
                risk_path = os.path.join(run_dir, "risk_mask_stats.json")
                with open(risk_path, "w", encoding="utf-8") as f:
                    json.dump(self.risk_log_history, f, ensure_ascii=False, indent=2)
                logger.info(f"Saved risk mask stats to: {risk_path}")
    ...
    def comput_custom_reg(self, model, reg_lambda=0.1):
        ...
        for lora_A_name, A in dict_A.items():
            ...
            if rank_mask is not None:
                mA = rank_mask.to(A.device, dtype=A.dtype).view(-1, 1)
                mB = rank_mask.to(B.device, dtype=B.dtype).view(1, -1)
                BA = (B * mB) @ (A * mA)
            else:
                BA = (B @ A)

            reg_loss += torch.norm(M * BA, p=2)
            param_count += 1
