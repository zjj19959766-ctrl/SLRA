...
# ============================================================
# [消融实验] Patch 训练 + 无 SVD 吸收
# 基于：LoRASculpt_Trainer3.1.py
#
# 修改内容（仅此一处，其余与 v3.1 完全相同）：
#   - 保留：AB_masks 幅度稀疏化
#   - 保留：CMR HOG 正则化
#   - 保留：Patch 挂载 + 训练（相同步数、调度器、梯度累加）
#   - 移除：SVD 吸收合并步骤
#   - 替换为：清除 Forward Hook 后直接 save_model
#
# 实验目的：
#   隔离 SVD 吸收对最终性能的独立贡献。
#   对比 v3.1（有 SVD）与本文件（无 SVD）的性能差距，
#   直接证明 SVD 吸收是核心关键步骤，而非 Patch 训练步数带来的提升。
# ============================================================
...
class LoRASculpt(LLaVATrainer):
    ...
    def _inner_training_loop(
        ...
        # ============================================================
        # 第三步：Patch 训练阶段（与 v3.1 完全相同）
        # [消融] SVD 吸收已替换为直接清除 Hook + save_model
        # ============================================================
        try:
            ...
            if patch_params:
                ...
                patch_optimizer = torch.optim.AdamW([{'params': patch_params, 'lr': 5e-4}])

                import transformers
                scheduler = transformers.get_cosine_schedule_with_warmup(
                    patch_optimizer,
                    num_warmup_steps=int(refine_steps * 0.1),
                    num_training_steps=refine_steps
                )
                ...
                for step in range(refine_steps):
                    ...
                    if step % 50 == 0 or step == refine_steps - 1:
                        logger.info(
                            f"   [Patch Refine] Step {step}/{refine_steps}: "
                            f"Loss {(loss * grad_acc_steps).item():.4f} | "
                            f"LR: {scheduler.get_last_lr()[0]:.6f}"
                        )

                # ============================================================
                # [消融] 移除 SVD 吸收，改为：清除 Hook 直接保存
                # v3.1 原本在此处做 SVD 分解并写回主 LoRA 权重
                # 本消融保持 Patch 权重存在于 Hook 中，不合并，直接保存主模型
                # 注意：由于 Hook 无法随 save_model 持久化，
                #       Patch 学到的知识实际上会丢失——这正是我们要验证的
                # ============================================================
                logger.info("[ablation] Skipping SVD absorption. Clearing hooks and saving main LoRA only.")
                with torch.no_grad():
                ...
                logger.info("✅ Patch Training Done (no SVD). Hooks cleared. Main LoRA saved as-is.")
